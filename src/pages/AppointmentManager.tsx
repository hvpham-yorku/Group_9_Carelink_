...
import { CalendarDays, CheckCircle2, Circle, Plus, Trash2, Pencil } from "lucide-react";
...
export default function AppointmentManager() {
     const { user } = useAuth();
     const { selectedPatientId, careTeamId, loading: patientLoading } = usePatient();
     ...
     const handleToggleComplete = async (appointment: AppointmentRecord) => {
          try {
               ...
               if (appointment.completedTime) {
                    await appointmentService.markAppointmentIncomplete(appointment.appointmentId);
               } else {
                    await appointmentService.markAppointmentComplete(appointment.appointmentId);
               }
                         ...
                         <div className="card border-0 shadow-sm rounded-4 h-100">
                              <div className="card-body p-4">
                                   ...
                                   {appointments.length === 0 ? (
                                        ...
                                        Object.entries(groupedAppointments).map(([monthLabel, items]) => (
                                             <div key={monthLabel} className="mb-4">
                                                  <div className="fw-semibold text-primary mb-2">{monthLabel}</div>
                                                       ...
                                                       {items.map((appointment) => {
                                                            ...
                                                            return (
                                                                 <button
                                                                      ...
                                                                      type="button"
                                                                      className={`btn text-start border rounded-4 p-3 ${isSelected ? "border-primary bg-primary bg-opacity-10" : "bg-white"
                                                                           }`}
                                                                 ...
                                                                 >
                                                                      <div className="d-flex justify-content-between align-items-start">
                                                                           ...
                                                                           <div className="text-end ms-3">
                                                                                ...
                                                                                <div className="small text-secondary">
                                                                                     {appointment.completedTime ? "Completed" : "Scheduled"}
                                                                                </div>
                                                                           </div>
                                                                      </div>

                                                                      <div className="mt-2 small">
                                                                           {appointment.description?.trim() || "No description provided."}
                                                                      </div>
                                                                 </button>
                                                            );
                                                       })}
                         ...
                         <div className="card border-0 shadow-sm rounded-4 h-100">
                              <div className="card-body p-4">
                                   ...
                                   {upcomingAppointments.length === 0 ? (
                                        <p className="text-secondary mb-0">No upcoming appointments yet.</p>
                                   ) : (
                         ...
                         <div className="card border-0 shadow-sm rounded-4 h-100">
                              <div className="card-body p-4">
                                   ...
                                   {showForm ? (
                                        ...
                                        <>
                                             <div className="d-flex align-items-center gap-2 mb-3">
                                                  {selectedAppointment.completedTime ? (
                                                       <CheckCircle2 size={20} className="text-success" />
                                                  ) : (
                                                       <Circle size={20} className="text-secondary" />
                                                  )}
                                                  <span className="fw-semibold">
                                                       {selectedAppointment.completedTime ? "Completed" : "Scheduled"}
                                                  </span>
                                             </div>
                                             ...
                                             <div className="mb-3">
                                                  ...
                                                  <div>{selectedAppointment.description?.trim() || "No description provided."}</div>
                                             </div>
                                             ...
                                             <div className="mb-4">
                                                  ...
                                                  <div>
                                                       {selectedAppointment.completedTime
                                                            ? formatDateTime(selectedAppointment.completedTime)
                                                            : "Not completed yet"}
                                                  </div>
                                             </div>
                                             ...
                                             <div className="d-flex flex-wrap gap-2">
                                                  ...
                                                  <button
                                                       type="button"
                                                       className={`btn rounded-pill d-inline-flex align-items-center gap-2 ${selectedAppointment.completedTime
                                                                 ? "btn-outline-warning"
                                                                 : "btn-outline-success"
                                                            }`}
                                        ...
                                        <p className="text-secondary mb-0">
                                             Select an appointment from the calendar or upcoming list, or add a new one.
                                        </p>
                                   )}
                              </div>
                         </div>
